part-Ptwo-one = {
  \key bes \major
  \time 4/4
  \clef treble
  r4 r8\mp bes16 c'16 d'8 f'4 bes'8~ |
  bes'4 r8 bes'16 a'16 g'8 f'4 d'8~ |
  d'4 r8 bes16 c'16 d'8 f'4 f'8~ |
  f'4 r8 bes16 c'16 d'8 f'4 f'8~ |
  f'4 r8 c'16 d'16 ees'8 f'8 r8 a'8~ |
  % 5
  a'8 a'8 a'8 c'16 d'16 ees'8 f'4 f'8~ |
  f'4 r8 c'16 d'16 ees'8 f'8 r8 a'8~ |
  \time 2/4
  a'8 a'8 a'8 c'16 d'16 |
  \time 4/4
  ees'8\f d'4 d'8~ d'4 r4 |
  R1-\markup { "Speech!" } |
  % 10
  R1 |
  r4 r8 bes16\f c'16 d'8 f'4 bes'8~ |
  bes'4 r8 bes'16 a'16 g'8 f'4 g'8~ |
  g'8 d'8 f'8 bes16 c'16 d'8 f'4 f'8~ |
  f'4 r8 bes16 c'16 d'8 f'4 f'8~ |
  % 15
  f'4 r8 c'16 d'16 ees'8 f'8 r8 a'8~ |
  a'8 a'8 a'8 c'16 d'16 ees'8 f'4 f'8~ |
  f'4 r8 c'16 d'16 ees'8 f'8 r8 a'8~ |
  \time 2/4
  a'8 a'8 a'8 c'16 d'16 |
  \time 4/4
  ees'8 d'4 d'8~ d'4 r8 c'16 d'16 |
  % 20
  ees'8\f f'4 f'8~ f'2~ |
  f'4 r4 r4 r8 c'16 d'16 |
  ees'8 f'4 f'8~ f'2~ |
  f'4 r4 r2 |
  g8\ben-mf c'16 c'16~ c'8 d'8~ d'8 c'8~ c'4 |
  % 25
  r2 r4 r8. c'16 |
  f8 bes16 bes16~ bes16 bes16 c'8~ c'8 bes8~ bes4 |
  R1 |
  f8 bes16 bes16~ bes16 bes16 c'8~ c'8 bes8~ bes4 |
  r2 r4 r8. c'16 |
  % 30
  f8 bes16 bes16~ bes16 bes16 c'8~ c'8 bes8~ bes4 |
  R1 |
  R1 |
  R1 |
  R1 |
  % 35
  r8 g'8\f g'8 g'16 g'16 g'8 a'16 bes'16~ bes'8 r8 |
  r2 r4 r8 c'16 d'16 |
  ees'8 f'4 f'8~ f'2 |
  r2 r4 r8 c'16 d'16 |
  ees'8 f'4 f'8~ f'2 |
  % 40
  r4 r8 bes16 \f c'16 d'8 f'4 bes'8~ |
  bes'4 r8 bes'16 a'16 g'8 f'4 g'8~ |
  g'8 d'8 f'8 bes16 c'16 d'8 f'4 f'8~ |
  f'4 r8 bes16 c'16 d'8 f'4 f'8~ |
  f'4 r8 r8 r2 |
  % 45
  R1 |
  \time 4/4
  R1 |
  \time 2/4
  r4 r8 c'16 d'16 |
  \time 4/4
  ees'8 d'4 d'8~ d'4 r8 c'16 d'16 |
  \time 4/4
  ees'8 f'4 f'8~ f'2~ |
  % 50
  f'4 r2 r4 |
  R1 |
  R1 |
  R1 |
  R1 |
  % 55
  R1 |
  \time 2/4
  R2 |
  \time 4/4
  r2 r2 \fermata |
}

rosie_lyr =  \lyricmode {
  If you change your mind, 
  I'm the first in line, 
  Ho -- ney I'm still free, 
  Take a chance on me!

  If you need me, let me know, 
  Go -- nna be a -- round,
  If you've got no place to go,
  If you're fee -- ling down.

  If you're all a -- lone, 
  When the pre -- tty birds have flown, 
  Ho -- ney I'm still free,
  Take a chance on me!

  Go -- nna do my ve -- ry best,
  And it ain't no lie,
  If you put me to the test, 
  If you let me try

  Take a chance on me,
  Take a chance on me

  We can go dan -- cing,
  As long as we're to -- get -- her.
  Lis -- ten to some mu -- sic
  You'd get to know me bet -- ter.

  You want me to leave it there
  But I think you know,
  That I cant let go!

  If you're all a -- lone,
  When the pre -- tty birds have flown, 
  Ho -- ney I'm still free,
  Take a chance on me!
  If you let me try.

  Take a chance on me!
}